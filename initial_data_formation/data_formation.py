...
from sqlalchemy.types import Integer, BigInteger, VARCHAR

users_df = pd.read_csv("initial_data_formation/users.csv", usecols=["id", "telegram_chat_id", "username", "full_name", "city"])
...
users_df.to_sql('user', engine, dtype={"chat_id": BigInteger(), "notion_id": VARCHAR(), "full_name": VARCHAR(), "username": VARCHAR(), "region": Integer()}, index=False, if_exists="append")